...
Deno.test('past the halfway point the short way is backwards', () => {
  const nearlyAround = torusToHexagon({
    floor: 0n,
    row: 0n,
    column: TORUS_COLUMNS - 1n,
  })
  assertEquals(displacement(nearlyAround), [
    { direction: 'west', hexagons: 1n, decimetres: 52n },
  ])
  const oneRowBack = torusToHexagon({
    floor: 0n,
    row: TORUS_ROWS - 2n,
    column: 0n,
  })
...
Deno.test('distances scale from metres to light-years', () => {
  ...
  assert(
    formatDistance(94_607_304_725_808_000n * 3n).startsWith('3 light-years'),
  )
})

Deno.test('nearby hexagons to search follows displayed direction imprecision', () => {
...
Deno.test('walking times scale from minutes to universe-ages', () => {
  ...
  const crossing = formatDuration(
    walkSeconds(94_607_304_725_808_000n * 10n ** 100n),
  )
